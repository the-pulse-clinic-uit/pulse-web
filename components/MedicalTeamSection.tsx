...
const doctors: Doctor[] = [
  { name: "Dr. Clara Bellweathe", specialty: "Chief Medical Officer", image: doctor1 },
  { name: "Dr. Olivia Sterling", specialty: "Cardiology Specialist", image: doctor2 },
  { name: "Dr. Robert Anderson", specialty: "Senior Physician", image: doctor3 },
  { name: "Dr. James Sullivan", specialty: "Internal Medicine", image: doctor4 },
];
...
const MedicalTeamSection: React.FC<MedicalTeamSectionProps> = ({ id = "team" }) => {
  return (
    <section id={id} className="py-20 lg:py-32 bg-background scroll-mt-24">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
          <span className="text-primary font-semibold text-lg">Meet Our Experts</span>
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-foreground">
            Our Medical Team
          </h2>
          <p className="text-muted-foreground text-lg">
            A team of more than 1,000 compassionate healthcare professionals, each dedicated to
            providing you with the highest quality medical care and personalized attention.
          </p>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {doctors.map((doctor, index) => {
            const delayStyle: CSSProperties = { animationDelay: `${index * 0.1}s` };

            return (
              <div key={doctor.name} className="group text-center animate-fade-in" style={delayStyle}>
